...
public class fran_bolts {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < k; i++) {
			...
			for(int j = 0; j < l; j++) {
				boltsPackage[i][j] = reader.nextInt();
			}
		}
		solve();
		reader.close();
		...
		if(i == 0) {
			...
			while(true) {
				...
				for(int j = 0; j < boltsPackage[i - 1].length; j++) {
					if(v - boltsPackage[i - 1][j] >= 0 && minbolts.get(v - boltsPackage[i - 1][j]) != Long.MAX_VALUE) {
						min = Math.min(min, minbolts.get(v - boltsPackage[i - 1][j]) + boltsReality[i - 1][j]);
					}
				}
				minbolts.add(min);
