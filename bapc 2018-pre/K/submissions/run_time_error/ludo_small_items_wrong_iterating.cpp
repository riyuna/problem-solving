...
int main()
{
	...
	for (int i = 1; i < K; i++) {
		...
		int boltsum = 0, pkg = 0;
		while (pkg < (int) sizes[i].size() && boltsum < (int) A.size()) {
			if (boltsum < sizes[i][pkg].x)
				boltsum = sizes[i][pkg].x;
			while (boltsum < (int) A.size() && A[boltsum] == INF)
				boltsum++;
			if (boltsum == (int) A.size()) {
				// in a range of size msize, all were INF. Thus, previous items
				// are bigger than now.
				break;
			}
			sizes[i][pkg].y = A[boltsum];
			pkg++;
		}

		// ASSUMPTION: for all j, sizes[i][j] is different
		for (int ppkg = 0; pkg < (int) sizes[i].size() && ppkg < (int) sizes[i-1].size(); ppkg++) {
