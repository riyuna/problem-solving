...
int main() {
    ...
    //cout << "I've found " << set_ct << " singles/pairs." << endl;
    for(int i = 0; i < N; i++) {
        if(v[i]) {
            w.push_back(v[i]);
        }
    }
    //cout << "Leaving " << w.size() << " people." << endl;
    ...
    for(int i = 0; i < (1 << w.size()); i++) {
        ...
        //cout << "Bitmask " << i << " has sum " << sum << endl;
        if(sum == 0) 
    ...
    for(int i = 0; i < S; i++) {
        for(int j = 0; j < i; j++) {
            if(!(zero_sets[j] & ~zero_sets[i])) { // If j is a submask of i...
                groups[i] = max(groups[i], groups[j] + 1);
                //cout << " to " << groups[i] << endl;
            }
        }
    }
